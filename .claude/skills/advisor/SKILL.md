---
name: advisor
description: Analyze subagent performance metrics and suggest new agents or improvements. Run this after using Claude Code for a while to get optimization suggestions visible in the dashboard.
user_invocable: true
---
...
If the server is not running or returns an error, start it first:
```bash
